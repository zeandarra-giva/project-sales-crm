...
var handler2 = async (req, ctx) => {
  try {
    ...
    logger2.info("Created new deal", { dealId: newDeal.id, bdId: user.id });
    return {
      status: 201,
      body: {
        ...newDeal,
    ...
    logger2.warn("Failed to create deal", { error: error.message });
    return {
      status: error.message === "Not authenticated" ? 401 : 500,
      body: { error: error.message }
    };
...
var config3 = {
  ...
  triggers: [{
    ...
    bodySchema: z2.object({
      ...
      decisionMakerTier: z2.number().min(1).max(5).optional(),
...
var handler3 = async (req, ctx) => {
  try {
    ...
    logger3.info("Contact updated", { contactId: id, by: user.id });
    ...
    logger3.error("Failed to update contact", { error });
...
var handler4 = async (req, ctx) => {
  try {
    const user = await authenticate(req.request);
    logger4.info("Listing contacts", { userId: user.id });
    ...
    logger4.error("Failed to list contacts", { error });
    return { status: 500, body: { error: "Internal server error" } };
  }
};
...
var config5 = {
  ...
  triggers: [{
    ...
    bodySchema: z3.object({
      ...
      decisionMakerTier: z3.number().min(1).max(5).default(3),
...
var handler5 = async (req, ctx) => {
  try {
    ...
    logger5.info("Contact created", { contactId: contact.id, by: user.id });
    ...
    if (error instanceof Prisma2.PrismaClientValidationError || error instanceof Prisma2.PrismaClientKnownRequestError && error.code === "P2000") {
      return { status: 400, body: { error: "Invalid input \u2014 check field lengths and types" } };
    }
    logger5.error("Failed to create contact", { error });
    return { status: 500, body: { error: "Internal server error" } };
  }
};
...
var config6 = {
  ...
  triggers: [{
    ...
    bodySchema: z4.object({
      ...
      accountType: z4.enum(["ENTERPRISE", "CORPORATE", "SMB", "GOVERNMENT"]).optional(),
      status: z4.enum(["ACTIVE", "INACTIVE", "PROSPECT"]).optional(),
      industryId: z4.string().optional(),
...
var handler6 = async (req, ctx) => {
  try {
    ...
    logger6.info("Client updated", { clientId: id, by: user.id });
    return { status: 200, body: updated };
  } catch (error) {
    logger6.error("Failed to update client", { error: error.message, clientId: req.request.pathParams?.id });
    if (error.name === "AuthError") {
      return { status: 401, body: { error: error.message } };
    }
...
var handler7 = async (req, ctx) => {
  try {
    const user = await authenticate(req.request);
    logger7.info("Listing clients", { userId: user.id });
    ...
    logger7.error("Failed to list clients", { error });
    return { status: 500, body: { error: "Internal server error" } };
  }
};
...
var handler8 = async (req, ctx) => {
  try {
    ...
    logger8.error("Failed to get client details", { error: error.message, clientId: req.request.pathParams.id });
    return {
...
var config9 = {
  ...
  triggers: [{
    ...
    bodySchema: z5.object({
      ...
      accountType: z5.enum(["ENTERPRISE", "CORPORATE", "SMB", "GOVERNMENT"]),
      status: z5.enum(["ACTIVE", "INACTIVE", "PROSPECT"]).default("PROSPECT"),
      industryId: z5.string().optional(),
      referralId: z5.string().optional()
    })
  }],
  enqueues: [],
  flows: ["sales-pipeline"]
...
var handler9 = async (req, ctx) => {
  try {
    ...
    logger9.info("Client created", { clientId: client.id, by: user.id });
    ...
    if (
      // ← Refactor 4
      error instanceof Prisma4.PrismaClientKnownRequestError && (error.code === "P2025" || error.code === "P2003")
    ) {
      return { status: 400, body: { error: "Related record not found (check industryId, referralId)" } };
    }
    logger9.error("Failed to create client", { error });
    return { status: 500, body: { error: "Internal server error" } };
  }
};
...
var handler10 = async (req, ctx) => {
  try {
    const user = await authenticate(req.request);
    logger10.info("Auth check successful", { userId: user.id });
    return {
      status: 200,
      body: { user }
    };
  } catch (error) {
    logger10.warn("Auth check failed", { error: error.message });
    return {
      status: 401,
...
var handler11 = async (req, ctx) => {
  ...
  logger11.info("Login attempt", { email });
  const bd = await prisma.bD.findUnique({
    where: { email }
  });
  if (!bd) {
    logger11.warn("Login failed - user not found", { email });
    return {
      status: 401,
  ...
  if (!bd.isActive) {
    logger11.warn("Login failed - account deactivated", { email });
    return {
      status: 401,
  ...
  if (!passwordValid) {
    logger11.warn("Login failed - wrong password", { email });
    return {
      status: 401,
      body: { error: "Invalid email or password" }
    };
  ...
  logger11.info("Login successful", { email, role: bd.role });
  return {
    status: 200,
    body: {
      token,
...
motia.addStep(config3, "./steps/api/contacts/update.step.ts", handler3, "./steps/api/contacts/update.step.ts");
motia.addStep(config4, "./steps/api/contacts/list.step.ts", handler4, "./steps/api/contacts/list.step.ts");
motia.addStep(config5, "./steps/api/contacts/create.step.ts", handler5, "./steps/api/contacts/create.step.ts");
motia.addStep(config6, "./steps/api/clients/update.step.ts", handler6, "./steps/api/clients/update.step.ts");
motia.addStep(config7, "./steps/api/clients/list.step.ts", handler7, "./steps/api/clients/list.step.ts");
motia.addStep(config8, "./steps/api/clients/detail.step.ts", handler8, "./steps/api/clients/detail.step.ts");
motia.addStep(config9, "./steps/api/clients/create.step.ts", handler9, "./steps/api/clients/create.step.ts");
motia.addStep(config10, "./steps/api/auth/me.step.ts", handler10, "./steps/api/auth/me.step.ts");
motia.addStep(config11, "./steps/api/auth/login.step.ts", handler11, "./steps/api/auth/login.step.ts");
motia.initialize();
